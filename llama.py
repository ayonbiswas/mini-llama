...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        """
        # todo
        # if x is of shape Bx C x h1 x h2 x ....
        #average over C x h1 x h2...; each sample will have a norm 
        # get last d - 1 dims
        norm_dim = x.shape[1:]
        rms = torch.sqrt(torch.norm(x, dim=norm_dim)/torch.prod(norm_dim) + self.eps ) #shape : B
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        # q, k, v : B x hds x seq x dim

        # qkT
        key = torch.transpose(key, -1, -2)
        att = (query @ key)*1/torch.sqrt(self.head_dim) # B x hds x seq x seq
    ...
    def forward(
        ...
        query = self.compute_query(x) # B x seq x emb
        key = self.compute_key(x)
        value = self.compute_value(x)
        query = query.view(batch_size, seqlen, self.n_local_heads, self.head_dim)# B x seq x hx emb//h
...
class Llama(LlamaPreTrainedModel):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            logits = logits[:, -1, :] # crop to just the final time step # B x vocab
            # todo
            # raise NotImplementedError
            ...
            if temperature == 0.0:
                ...
                '''
                probs = nn.Softmax(logits/temperature, dim = -1)
